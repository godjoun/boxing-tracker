      ...
      >
        ...
        {isDone ? (
          <div className="gym-inquiry-done">
            ...
            <p>
              <strong>{gym.name}</strong> 체험 문의를 기록했습니다. 제휴 도장이
              연결되면 입력하신 연락처로 안내드릴게요.
            </p>
            {gym.phone ? (
            ...
            <button type="button" className="gym-inquiry-submit" onClick={onClose}>
              확인
            </button>
          </div>
        ) : (
          <form className="gym-inquiry-form" onSubmit={handleSubmit}>
            <p className="gym-inquiry-kicker">TRIAL INQUIRY</p>
            <h2 id="gym-inquiry-title">{gym.name} 체험 문의</h2>
            <p className="gym-inquiry-copy">
              입문·체험 상담을 요청해 주세요. 연락처는 앱에만 저장됩니다.
            </p>
