...
This repository is the current MANTLE boxing app.

The product name `MANTLE` is a working brand name and may change. Do not rename the product, package, routes, storage keys, analytics events, or public-facing assets unless the repository owner explicitly approves the final new name.
...
The current beachhead user is a person who wants to train boxing alone at home.

The current product is broader than a standalone round timer. It should help the user know what to do, complete the session with minimal interruption, and leave a useful record afterward.
...
6. Save the completed training automatically.
7. Let completed sessions accumulate into a meaningful boxer profile over time.

A simple shorthand for the current product promise is:

> Train boxing at home, and let the work you complete remain as a record.
...
The boxer is the protagonist. The product is the guide and tool.

Do not make the brand the hero with claims such as how revolutionary, elite, or impressive MANTLE is. The product should help the user act, train, and leave evidence of what they actually did.
...
The exact app name is currently under review. Naming exploration should support this association without locking the product into a narrow `boxing timer` category.
...
- a full rebrand implementation before the final app name is approved
