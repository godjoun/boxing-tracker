...
> **현재 백엔드 스택 = Supabase.** Firebase는 사용하지 않는다.  
> 인증·DB·Storage·권한·보안·마이그레이션 판단은 모두 Supabase 기준으로 한다.
...
- **지도 입점관 좌표 추가:** `#2 입점관리` → **`#11 베타RLS` 다시 실행**
...
| 기존 입점관이 지도 핀 없이 아래 목록에만 보임 | 관 정보 수정에서 주소 검색 → 지도 위치 선택 → 저장 |
